...
export default function DragAndDrop({
  ...
  // sync props -> state while preserving ids for stable drag
  useEffect(() => {
    setItems((prev) => {
  ...
  function handleAddLesson() {
    const nextIndex = items.length + 1;
    const newItem = { id: `new-${Date.now()}`, title: `Урок ${nextIndex}` };
    const next = [...items, newItem];
    setItems(next);
    onChange?.(next.map((x) => x.title));
    addModuleHandleClick?.(newItem.title, next.map((x) => x.title));
  ...
  return (
    <div className="max-w-3xl">
      <ul
        ...
        className="space-y-2"
        role="list"
        aria-label="Список (перетягуйте для зміни порядку)"
      >
        {items.map((item) => (
          <li
            key={item.id}
              ...
              <>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="cursor-pointer inline-flex h-6 w-6 items-center justify-center rounded-full text-xs leading-none"
                  >
                    <img src={iconDelete} />
                  </button>
