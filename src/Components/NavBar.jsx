...
export default function Navbar() {
    ...
    <nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            ...
            <li>
              <div onClick={() => havigateToPage('/')} class={`cursor-pointer block py-2 px-3 text-gray-900 rounded-sm md:hover:bg-transparent md:hover:text-blue-700 md:p-0 ${location.pathname === '/' ? 'bg-blue-700 md:bg-transparent md:text-blue-700 md:dark:text-blue-500' : 'hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700'}`} aria-current={location.pathname === '/' ? 'page' : undefined}>Головна</div></li>
            <li>
               <div onClick={() => havigateToPage('/search')} class={`cursor-pointer block py-2 px-3 text-gray-900 rounded-sm md:hover:bg-transparent md:hover:text-blue-700 md:p-0 ${location.pathname === '/search' ? 'bg-blue-700 md:bg-transparent md:text-blue-700 md:dark:text-blue-500' : 'hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700'}`} aria-current={location.pathname === '/search' ? 'page' : undefined}>Курси</div></li>
            <li>
            <div onClick={() => havigateToPage('/certificate')} class={`cursor-pointer block py-2 px-3 text-gray-900 rounded-sm md:hover:bg-transparent md:hover:text-blue-700 md:p-0 ${location.pathname === '/certificate' ? 'bg-blue-700 md:bg-transparent md:text-blue-700 md:dark:text-blue-500' : 'hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700'}`} aria-current={location.pathname === '/certificate' ? 'page' : undefined}>Сертифікати</div></li>
